...
//  $vars['styles'] = drupal_get_css();
  ...
  if (!$vars['is_front']) {
    ...
    if($vars['node']->type == 'page') {
      ...
      if($sImg = $vars['node']->field_sidebar_image[0]['view']) {
        $vars['right'] = '<div class="field-side-img">' . $sImg . '</div>' . $vars['right'];
        $body_classes[] = 'two-sidebars';
        }
      
      if($quote_footer = $vars['node']->field_quote_footer[0]['view']) {
        $vars['content_bottom'] = '<div class="field-quote-bottom">' . $quote_footer . '</div>' . $vars['content_bottom'];
      }
