...
//     drupal_set_message('Please login First', 'warning');
  ...
  $form['purchaser']['field_first_name'] =   array(
      '#type'  =>'textfield',
      '#title'=> 'First Name',
      '#size'  => 20,
  ...
  $form['purchaser']['field_last_name'] =   array(
      '#type'  =>'textfield',
      '#title'=> 'Last Name',
      '#size'  => 20,
  ...
  $form['purchaser']['field_current_position'] =   array(
      '#type'  =>'textfield',
  ...
  $form['purchaser']['field_organization'] =   array(
      '#type'  =>'textfield',
  ...
  for($i=1; $i <= $licences_qnt; $i++)
  {
    ...
    $form['subscribers'][$sufix]['field_organization'.$sufix] = array(
        '#type'    =>'textfield',
        '#title'  => 'School Name',
        '#size'    => 20,
        '#required' => ($licences_qnt > 1) ? TRUE : FALSE,        
      );
    $form['subscribers'][$sufix]['field_first_name'.$sufix] = array(
        '#type'  =>'textfield',
        '#title'=> 'First Name',
        '#size'  => 20,
        '#required' => ($licences_qnt > 1) ? TRUE : FALSE,
      );
    $form['subscribers'][$sufix]['field_last_name'.$sufix] = array(
        '#type'  =>'textfield',
        '#title'=> 'Last Name',
        '#size'  => 20,
        '#required' => ($licences_qnt > 1) ? TRUE : FALSE,
      );  
    $form['subscribers'][$sufix]['field_current_position'.$sufix] = array(
        '#type'  =>'textfield',
        '#title'=> 'Current position/title',
        '#size'  => 20,
        '#required' => ($licences_qnt > 1) ? TRUE : FALSE,
...
/**
 * Altering registration form, adding new submit handler
 *
 * @param array $form
 * @param array $form_state
...
function cel_5d_course_registration_form_alter(&$form, $form_state, $form_id) {
  ...
  {  
    ...
    {
      $login_msg = 'If you already have an account please '.l('login', '5d-course/trial_login').' using your credentials. If you don’t already have an account with CEL, register below.';
      $form['msg'] = array( 
        '#markup' => t($login_msg),
        '#prefix' => '<p class="alert alert-info">',
        '#suffix' => '</p>',
        '#weight' => -20, 
      );
      $form['cel_5d_course_registration_target']   = array('#type'  => 'hidden', '#value'  => 'trial_registration');
      $form['#submit'][] = 'cel_5d_course_registration_user_register_form_submit';
    }
    elseif (arg(2) && arg(2) == 'course_registration')
    {
      ...
      $login_msg = 'If you already have an account please '.l('login', '5d-course/'.$module_nid.'/course_login').' using your credentials. If you don’t already have an account with CEL, register below.';
  ...
  elseif($form_id == 'user_login')
  {
    ...
    if(arg(1) && arg(1) == 'trial_login')
    {
      ...
      $login_msg = 'If you don’t have an account please '.l('register', '5d-course/trial/register').' first.';
      $form['msg'] = array(
    		'#markup' => t($login_msg),
    		'#prefix' => '<p class="alert alert-info">',
        '#suffix' => '</p>',
    ...
    elseif (arg(2) && arg(2) == 'course_login')
    {
      ...
      // add message on the top of the form
      $login_msg = 'If you don’t have an account please '.l('register', '5d-course/'.$module_nid.'/course_registration').' first.';
      $form['msg'] = array(
        '#markup'  => t($login_msg),
        '#prefix' => '<p class="alert alert-info">',
        '#suffix' => '</p>',
 ...
 * @return boolean
 */
function cel_5d_course_registration_user_register_form_submit($form, &$form_state)
{
  global $user;
  ...
  switch ($form_target_action) {
    case 'trial_registration':
      $role = user_role_load_by_name('5D Trial Temp');
